...
FAMILY_PATTERNS = {
    "PERMISSION_CLAIM": re.compile(r"管理员|最高权限|超级用户|授权用户|管理员权限|管理权限|系统权限|root(?:权限|用户)?", re.IGNORECASE),
    "SKIP_VERIFICATION": re.compile(r"(?:无需|不用|跳过)(?:验证|确认|校验|审核)|无需确认|不用确认|直接执行|直接操作"),
    "BYPASS_SAFETY": re.compile(r"忽略.{0,4}(?:安全|规则|限制)|绕过.{0,4}(?:安全|检查|校验)|关闭安全检查|不要检查安全|取消安全限制|跳过安全"),
