...
from app.models.schemas import (
    TextCommandRequest, TrustedRuntimeContext, VehicleState, VehicleStatePatch
)
...
def test_active_turn_is_protected_then_dynamic_stream_is_trimmed() -> None:
    ...
    for _ in range(20):
        repository.ingest_vehicle_state(state, None, "ACTIVE")
...
def test_one_thousand_turns_keep_repository_and_hnsw_bounded(pipeline) -> None:
    ...
    for _ in range(1000):
        last_result = pipeline.process_text(request, trusted_context=trusted)
    ...
    assert repository_status.static_node_count == 0
    latest_speed = next(
        node
