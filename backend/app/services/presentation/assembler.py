...
class PresentationAssembler:
        ...
        for intent_demand in demand.intent_demands:
            ...
            intent_presentations.append(
                IntentEvidenceDemandPresentation(
                    ...
                    knowledge_query_text=intent_demand.knowledge_query_text,
                    knowledge_retrieval_metadata=intent_demand.knowledge_retrieval_metadata,
                    knowledge_demand_sources=intent_demand.knowledge_demand_sources,
    ...
    def node_detail(self, record: AuditRecord, node_id: str) -> EvidenceNodeDetail | None:
        ...
        # 指令涉及的意图（整轮，与 knowledge_hits 一致展示）
        demand_by_intent = {
            item.intent_id: item
            for item in record.evidence_demand.intent_demands
            if item.intent_id
        }
        intent_ids = sorted(demand_by_intent.keys())
        # 安全知识命中取整轮（与法规一致）：该轮次所有意图的 augment 命中并集。
        # 避免「种子/全量状态节点」（DOOR_STATE/GEAR_STATE 等）不属于 resolution
        # 绑定时，节点详情看不到知识库检索结果。
        knowledge_hits: list[dict[str, Any]] = []
        for intent_id in sorted(demand_by_intent):
            demand_item = demand_by_intent[intent_id]
