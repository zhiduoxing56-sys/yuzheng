...
class EffectiveAuditResolver:
    ...
    def resolve(self, record: AuditRecord) -> EffectiveAuditResolution:
        ...
        if outcome is None:
            return EffectiveAuditResolution(record, None, record.final_decision)
        if outcome.original_turn_id != record.turn_id:
            raise ValueError("REVIEW_OUTCOME original_turn_id 与原始审计不一致")
        if outcome.original_final_decision != record.final_decision.final_decision:
            raise ValueError("REVIEW_OUTCOME original_final_decision 与原始审计不一致")
        merged = merge_decision(
            record.safety_gate_result,
            self._alignment_route(record),
            record.final_decision.score_decision,
