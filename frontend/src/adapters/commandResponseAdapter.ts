...
import { array, nonEmptyString, nullableString, number, record, stripSensitiveFields, string } from "./runtime";
...
function common(raw: unknown, inputType: InputType) {
  ...
  const safetyScore = number(decision.safety_score, "command.decision.safety_score");
  ...
  return { root, turnId, inputType, preliminaryDecision, safetyScore, action, target, auditId, safeResponse: stripSensitiveFields(raw) };
}
