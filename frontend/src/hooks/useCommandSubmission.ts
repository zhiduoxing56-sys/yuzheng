...
import type { SubmissionStatus, VehicleState } from "../types/contract";
...
export function useCommandSubmission({ sessionId, sessionEpoch, backendAvailable, onAccepted, onBeforeSubmit }: Options) {
  ...
  const sessionRef = useRef({ sessionId, sessionEpoch });
  sessionRef.current = { sessionId, sessionEpoch };

  useEffect(() => {
    controllerRef.current?.abort("session changed");
    ...
    return () => controllerRef.current?.abort("submission hook disposed");
  ...
  const run = useCallback(async (request: (signal: AbortSignal) => Promise<unknown>, adapt: (raw: unknown) => AdaptedCommandResponse) => {
    ...
    const isCurrent = () => generationRef.current === generation
      && sessionRef.current.sessionId === capturedSessionId
      && sessionRef.current.sessionEpoch === capturedSessionEpoch;
    ...
    try {
      ...
      setProcessingTurnId(result.turnId);
      ...
      if (controllerRef.current === controller && isCurrent()) controllerRef.current = null;
    }
  }, [backendAvailable, onAccepted, onBeforeSubmit, sessionEpoch, sessionId]);
  ...
  const reset = useCallback(() => {
    controllerRef.current?.abort("submission reset");
    controllerRef.current = null;
    const generation = ++generationRef.current;
    setSubmissionGeneration(generation);
    setStatus("idle");
  ...
  return { status, error, busy, immediateResult, processingTurnId, submissionGeneration, submitText, submitAudio, submitMicrophone, markCompleted, markPartial, markPresentationFailed, retryPresentation, reset };
}
