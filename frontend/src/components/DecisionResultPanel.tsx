...
export function DecisionResultPanel({ data, immediate, loading, error, onRetry }: Props) {
  ...
  const requiredEvidenceCount = data.evidence_demand.intent_demands.reduce((count, item) => count + item.required_types.length + (item.knowledge_required_types?.length ?? 0), 0);
  ...
  return <section className={`decision-card result-panel decision-${decision.final_decision.toLowerCase()}`}>
    ...
    {data.evidence_demand.intent_demands.some((item) => item.knowledge_augmented_types?.length) ? <div className="knowledge-augmented-block">
      {data.evidence_demand.intent_demands.map((intent) => intent.knowledge_augmented_types?.length ? <p key={intent.intent_id}><strong>📚 知识检索证据（缺失进入复核，不触发硬拒绝）· {intent.intent_id}</strong>：{intent.knowledge_augmented_types.join("、")}{intent.knowledge_hits?.length ? <span> · 命中 {intent.knowledge_hits.map((hit) => hit.title ?? hit.node_id).join("；")}</span> : null}</p> : null)}
    </div> : null}
