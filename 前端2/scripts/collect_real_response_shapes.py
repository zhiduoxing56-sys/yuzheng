...
def command(text: str, *, state: dict[str, Any] | None = None) -> tuple[int, Any]:
...
def main() -> None:
    ...
    pass_status, pass_result = command(
        "查询当前速度", state={"vehicle_speed": 0, "gear_position": "P"}
    )
    ...
    block_status, block_result = command(
        "打开车门", state={"vehicle_speed": 80, "gear_position": "D"}
    )
    ...
    execution_source_status, execution_source = command(
        "打开车门", state={"vehicle_speed": 0, "gear_position": "P"}
    )
