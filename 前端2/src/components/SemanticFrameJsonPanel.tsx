...
export function SemanticFrameJsonPanel({ data, loading, error, onRetry }: Props) {
  return <section className="decision-surface semantic-json-card" aria-labelledby="semantic-json-title">
    ...
    {data ? (
      <pre className="semantic-json-output" aria-label="语义帧 JSON">
        <code>{JSON.stringify(data.semantic_frame, null, 2)}</code>
      </pre>
    ) : error ? (
      <div className="decision-inline-state error" role="alert">
        <strong>语义帧暂时无法读取</strong>
        <p>{error}</p>
        <button type="button" onClick={onRetry}>重新读取</button>
      </div>
    ) : (
      <div className="decision-inline-state">
        <span className="semantic-placeholder-icon"><JsonIcon /></span>
        <strong>{loading ? "正在读取语义帧" : "解析结果将显示在这里"}</strong>
        <p>提交指令后，将原样展示后端返回的 JSON 结构。</p>
      </div>
    )}
  </section>;
}
