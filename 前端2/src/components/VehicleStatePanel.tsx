...
export function VehicleStatePanel({ data, semanticFrame, loading, error, onRefresh }: Props) {
  const target = semanticFrame?.target;
  const entries = data ? relevantVehicleStateEntries(data, target) : [];
  const availabilityMessage = vehicleStateAvailabilityMessage(target);
