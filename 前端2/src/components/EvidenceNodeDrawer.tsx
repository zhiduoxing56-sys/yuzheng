  ...
  return <div className="evidence-drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="evidence-node-drawer" role="dialog" aria-modal="true" aria-label="证据节点详情">
      ...
      {data && <div className="node-detail-content">
        ...
        <section><h4>质量信息</h4><dl className="quality-mini-grid"><div><dt>新鲜度</dt><dd>{formatPercent(data.freshness)}</dd></div><div><dt>一致性</dt><dd>{formatPercent(data.consistency)}</dd></div><div><dt>可用性</dt><dd>{formatPercent(data.availability)}</dd></div><div><dt>语义相关</dt><dd>{formatPercent(data.semantic_similarity)}</dd></div></dl></section>
        <section><h4>裁决关系</h4><ul className="plain-detail-list"><li>必查证据：{data.mandatory ? "是" : "否"}</li><li>后端明确引用：{isCritical ? "是" : "否"}</li><li>入边：{data.incoming_edges.length}；出边：{data.outgoing_edges.length}</li></ul></section>
        <section><h4>记忆与因果</h4><dl className="quality-mini-grid"><div><dt>初始记忆</dt><dd>{data.initial_memory_confidence === null ? "暂无" : formatPercent(data.initial_memory_confidence)}</dd></div><div><dt>最终记忆</dt><dd>{data.final_memory_confidence === null ? "暂无" : formatPercent(data.final_memory_confidence)}</dd></div><div><dt>因果支持</dt><dd>{data.causal_support === null ? "暂无" : formatPercent(data.causal_support)}</dd></div><div><dt>修正权重</dt><dd>{data.corrected_weight === null ? "暂无" : formatPercent(data.corrected_weight)}</dd></div></dl>{data.causal_parents.length > 0 && <p>因果父变量：{data.causal_parents.join("、")}</p>}</section>
