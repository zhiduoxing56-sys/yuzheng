...
import { getTurnPresentation, getTurnTimeline, getTurnWorkflowStatus } from "../api/turns";
...
export function prefetchReview(turnId: string): void {
  ...
  readCache.prefetch(readKeys.timeline(turnId), async (signal) => adaptTimeline(await getTurnTimeline(turnId, signal)));
}
