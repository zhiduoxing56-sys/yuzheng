...
describe("evidence layer adapter", () => {
  ...
  it("keeps unclassified nodes in global totals without guessing a layer", () => {
    const model = adaptEvidenceLayers(makeGraph([
      makeNode("A", { security_rank: null, mandatory: true }),
      makeNode("B", { security_rank: null, quality_label: "STALE", metadata: { display_name: "旧数据节点" } }),
    ], [makeEdge("C", "A", "B", { relation: "CONFLICTS" })]), null);
    expect(model.globalSummary).toMatchObject({ nodeCount: 2, hitCount: 2, mandatoryCount: 1, abnormalCount: 1, conflictNodeCount: 2, unclassifiedNodeCount: 2 });
    expect(model.layers.every((layer) => layer.nodes.length === 0)).toBe(true);
  ...
  it("uses mandatory supplement, abnormal, mandatory and critical preview priority", () => {
    const presentation = makePresentation({
      retrieval_summary: { ...makePresentation().retrieval_summary, mandatory_supplemented_node_ids: ["supplement"] },
    });
    const model = adaptEvidenceLayers(makeGraph([
      makeNode("plain", { semantic_similarity: 1 }),
      makeNode("critical"),
      makeNode("mandatory", { mandatory: true }),
      makeNode("abnormal", { quality_label: "STALE" }),
      makeNode("supplement"),
    ]), presentation, new Set(["critical"]));
    expect(model.layers[2].previews.map((node) => node.id)).toEqual(["supplement", "abnormal", "mandatory", "critical"]);
  });

  it("filters topK, supplement, mandatory, critical, abnormal, conflict and status independently", () => {
    const presentation = makePresentation({
      retrieval_summary: {
        ...makePresentation().retrieval_summary,
    ...
    const graph = makeGraph([
      makeNode("top"), makeNode("supplement"), makeNode("mandatory", { mandatory: true }),
    ...
    expect(visibleIds(model, { category: "mandatory" })).toEqual(["mandatory"]);
  ...
  it("combines search and filters without mutating original layers", () => {
    const model = adaptEvidenceLayers(makeGraph([makeNode("A", { source: "can", mandatory: true }), makeNode("B", { source: "camera" })]), null);
    const filtered = filterEvidenceLayers(model, { query: "can", category: "mandatory", status: "VALID" });
  ...
  it("keeps critical evidence separate from mandatory and conflict inference", () => {
    const presentation = makePresentation({
    ...
    const model = adaptEvidenceLayers(makeGraph([
      makeNode("mandatory", { mandatory: true }), makeNode("conflict"), makeNode("explicit"),
    ], [makeEdge("C", "mandatory", "conflict", { relation: "CONFLICTS" })]), presentation, deriveCriticalEvidenceIds(presentation));
    expect(model.nodesById.get("mandatory")?.isCritical).toBe(false);
