...
function command(decision = "PASS") {
  return { turn_id: "TURN_test", decision: { final_decision: decision }, semantic_frame: { intents: [{ action: "close", target: "reading_light" }] }, audit: {} };
}
...
function presentation(decision = "PASS", reviewStatus = "NOT_REQUIRED", candidates: unknown[] = [], auditId: unknown = "AUD_test") {
  ...
  return {
    ...
    evidence_demand: { intent_demands: [] }, retrieval_summary: { candidate_count: 0, missing_types: [], candidates: [] },
